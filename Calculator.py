...
def check_input(string):
    ...
    if re.search("[^0-9+\-*/^().]", string):
        print(
            "Please, enter a valid mathematical expression using only 0-9, +, -, *, ^, /, (, ) and , . ."
        )
        return False
    # String contains two operators in succession
    if re.search("[+\-*/^.]{2}", string):
        print(
            "There's an issue with your operators. Please check your sequence and try again."
        )
        return False
    # Parentheses close immdediately after opening
    if re.search("[(][)]", string):
        print(
            "Your parentheses do not contain a number or unary operator. Please check your sequence and try again."
        )
        return False
    # Parentheses close without operator afterwards
    if re.search("[)][^+\-*/^)(]", string):
        print(
            "Your parentheses close and are not followed by an operator.\nIf you wanted to multiply, please specify with operator '*'.\nPlease check your sequence and try again."
        )
        return False
    # Parentheses open without operator before
    if re.search("[^+\-*/^())][(]", string):
        print(
            "Your parentheses open and are not preceded by an operator.\nIf you wanted to multiply, please specify with operator '*'.\nPlease check your sequence and try again."
        )
        return False
        ...
        if char in "+-^*/.":
            if digit_flag:
                ...
                if char in ".":
                    if float_point_flag:
                        print(
                            "There's multiple float points on a single number. Please check your sequence and try again."
                        )
                        return False
                ...
                print(
                    "There's a stray operator with no second part. Please check your sequence and try again."
                )
                return False
    # String starts with / * ^ , or .
    if string[0] in "/*^.":
        print(
            "Your sequence starts with operator '",
            string[0],
            "'. Please check your sequence and try again.",
        )
        return False
    if check_zero_division(string):
        print(
            "Division by 0 not allowed. Please check your sequence and try again."
        )
        return False
    ...
    if (opening_match or closing_match) and len(opening_match) != len(
        closing_match
    ):
        print(
            "There's an issue with your parentheses. Please check your sequence and try again."
        )
        return False
...
def check_parentheses_sequence(my_string):
    ...
    for char in my_string:
        if char == ")":
            if current_open_parentheses == 0:
                print(
                    "Parentheses close without ever opening. Please check your sequence and try again."
                )
                return False
    ...
    # This means that parentheses are open
        ...
        print(
            "Parentheses open without ever closing. Please check your sequence and try again."
        )
        return False
...
def sequence_to_string(sequence_list, start, end):
    ...
    for operator_0 in operators:
        ...
        if operator_0 in sequence_list[start : end + 1]:
            ...
            operator_index = (
                sequence_list[start : end + 1].index(operator_0) + start
            )
            ...
            input_string = input_string.join(
                sequence_list[start_index : end_index + 1]
            )
            if (
                sequence_list[start_index - 1] == "("
                and sequence_list[end_index + 1] == ")"
            ):
                start_index -= 1
                end_index += 1
...
def calculate(string):
    ...
    if "^" in string:
        ...
        result = pow(
            float(parts_to_calculate[0]), float(parts_to_calculate[1])
        )
    elif "/" in string:
        parts_to_calculate = string.split("/")
    ...
    # and find actual two-part operation
        ...
        # there is also a 2nd part
            ...
            result = float(parts_to_calculate[0]) + float(
                parts_to_calculate[1]
            )
...
def is_result_infinity(result):
    ...
    if result == "infinity":
        print(
            "Calculations result in division by 0, which is notallowed.\nPlease check your sequence and try again."
        )
        flag = True
    return flag
...
while __name__ == "__main__":
    user_input = input(
        "Give me a mathematical expression to solve or 'exit' to terminate: \n"
    )
